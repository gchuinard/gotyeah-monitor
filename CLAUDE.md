...
**Team deletion is deferred (soft-delete), not immediate.** `DELETE /teams/{id}` (admin) sets `Team.deletion_scheduled_at = now` instead of deleting — refused (409) if it's the user's last **active** (non-scheduled) team. A scheduled team is **deactivated** (the loop's `_run_one_cycle` filters its monitors out of `due`, so no checks/alerts/incidents) but still listed, with a **reactivate** path: `POST /teams/{id}/restore` clears the flag. The loop's `_run_team_purge` (retention cadence) hard-deletes teams past `TEAM_DELETION_GRACE_DAYS = 7` (cascade). **Gotcha:** `Team.monitors`/`monitor_groups`/`members` set **`passive_deletes=True`** so a `session.delete(team)` relies on the DB's `ON DELETE CASCADE` instead of SQLAlchemy's default "disassociate" (which would `UPDATE ... SET team_id=NULL` and orphan the rows). Front: a "Zone de danger" in the team modal with a typed-name confirmation popup, an amber "suppression programmée" banner + Réactiver button when scheduled, and the header switcher auto-falls-back to another active team.
...
- **There are no automated tests** in this repo, neither API nor front (checked 2026-09-25). `backend` only installs deps and runs `compileall` under Python 3.11, same as prod (it ran 3.14 until 2026-09-25, so 3.12+ syntax could pass CI and break prod), `frontend` runs lint and build. A green CI says nothing about behaviour: don't claim a change is "tested by CI".
